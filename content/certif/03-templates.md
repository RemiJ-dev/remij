...
- [X] **B.** `composer require symfony/twig-bundle`
...
- [X] **A.** `{{ ... }}` — afficher le contenu d'une variable ou le résultat d'une expression
- [X] **B.** `{% ... %}` — exécuter de la logique (condition, boucle…)
- [X] **C.** `{# ... #}` — ajouter des commentaires
...
- [X] **C.** Ils ne sont pas inclus dans la page rendue, contrairement aux commentaires HTML
...
- [X] **D.** Non — Twig fournit ses propres utilitaires (filtres, fonctions, tags) pour exécuter de la logique

### Question 5

Quelles affirmations sur les performances de Twig sont vraies ? *(plusieurs bonnes réponses)*

- [X] **A.** En `prod`, les templates sont compilés en PHP et mis en cache automatiquement
- [X] **B.** En `dev`, les templates sont recompilés automatiquement quand on les modifie
...
- [X] **A.** Il applique le filtre `upper` : le contenu de `title` est mis en majuscules avant affichage
...
- [X] **A.** Utiliser le snake case pour les noms de fichiers et de dossiers (ex. `blog_posts.html.twig`)
- [ ] **B.** L'extension `.twig` est optionnelle
- [X] **C.** Définir deux extensions pour les noms de fichiers (ex. `index.html.twig`)
- [X] **D.** La première extension (`html`, `xml`…) est le format final que le template va générer

### Question 9

Dans quel répertoire les templates sont-ils stockés par défaut ? *(une seule bonne réponse)*

- [X] **A.** `templates/` à la racine du projet
...
- [ ] **A.** `$foo->getBar()` (getter)
- [X] **B.** `$foo['bar']` (tableau et élément)
...
- [X] **A.** Par défaut, la valeur `null` est utilisée
- [X] **B.** Si l'option `strict_variables` est activée, une exception `Twig\Error\RuntimeError` est levée
...
- [X] **D.** Elle génère l'URL relative de la route à partir de son nom et de ses paramètres
...
- [X] **A.** `url()`, qui prend les mêmes arguments que `path()`
...
- [X] **A.** Elle gère le versioning des assets (hash ajouté aux URLs pour le cache busting)
- [X] **B.** Elle nécessite d'installer le paquet `symfony/asset`
- [ ] **C.** Elle génère toujours des URLs absolues
- [X] **D.** Elle rend l'application portable : le chemin généré s'adapte si l'app est hébergée dans un sous-répertoire
...
- [X] **D.** `{{ absolute_url(asset('images/logo.png')) }}`
...
- [X] **A.** `app.request` — l'objet `Request` courant
- [ ] **B.** `app.container` — le container de services
- [X] **C.** `app.user` — l'utilisateur courant, ou `null` s'il n'est pas authentifié
- [X] **D.** `app.flashes` — les messages flash stockés en session

### Question 19

De quelle classe la variable `app` est-elle une instance ? *(une seule bonne réponse)*

- [X] **A.** `Symfony\Bridge\Twig\AppVariable`
- [ ] **B.** `Twig\AppVariable`
...
- [X] **B.** `app.environment` — le nom de l'environnement courant (`dev`, `prod`…)
- [X] **C.** `app.debug` — `true` en mode debug
- [X] **D.** `app.current_route` — le nom de la route courante

### Question 21

Quelles affirmations sur les variables globales définies dans `twig.globals` sont vraies ? *(plusieurs bonnes réponses)*

- [X] **A.** Elles sont injectées automatiquement dans tous les templates Twig
- [X] **B.** Un service se référence en préfixant son id par `@` : `uuid: '@App\Generator\UuidGenerator'`
- [ ] **C.** Les services référencés sont chargés en lazy : ils ne sont instanciés que si la variable est utilisée
- [X] **D.** Elles peuvent contenir des valeurs statiques, ex. `ga_tracking: 'UA-xxxxx-x'`
...
- [X] **B.** C'est une convention **optionnelle** pour distinguer les fragments des templates complets
...
- [X] **A.** Le template inclus a accès à toutes les variables du template qui l'inclut
- [X] **B.** Ce comportement se contrôle avec l'option `with_context`
- [X] **C.** On peut passer (et donc renommer) des variables : `{{ include('blog/_user_profile.html.twig', {user: blog_post.author}) }}`
...
- [X] **C.** `base.html.twig` (éléments communs) → `layout.html.twig` (structure du contenu) → les pages de l'application
...
- [X] **A.** Le tag `{% block %}` définit les sections qu'un template enfant peut surcharger
- [ ] **B.** Un template enfant doit surcharger **tous** les blocks définis par son parent
- [X] **C.** Un block peut être vide, comme `{% block content %}{% endblock %}`
- [X] **D.** Un block peut définir un contenu par défaut, affiché quand les enfants ne le surchargent pas
